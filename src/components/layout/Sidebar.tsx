        ...
        <div className="flex-1 flex flex-col overflow-hidden px-3">
          ...
          <div className="mt-4 bg-[#111111]/80 rounded-2xl p-1.5 border border-white/5 flex gap-1">
            {AVAILABLE_MODELS.map((model) => {
              ...
              const isLocked = isThinking && thinkingCooldownUntil > Date.now();
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex-1 flex items-center justify-center gap-2 py-2 rounded-xl text-[13px] font-semibold transition-all relative overflow-hidden",
                    isLocked 
                      ? "opacity-60 cursor-not-allowed bg-white/[0.02] text-zinc-500 border border-amber-500/20" 
                      : "cursor-pointer",
                    !isLocked && isSelected 
                      ? isThinking 
                        ? "bg-indigo-500/20 text-indigo-300 shadow-[inset_0_1px_1px_rgba(255,255,255,0.1)] border border-indigo-500/30"
                        : "bg-white/10 text-white shadow-[0_0_15px_rgba(255,255,255,0.15)] border border-white/25"
                      : !isLocked ? "text-zinc-500 hover:text-zinc-300 hover:bg-white/5 border border-transparent" : ""
                  )}
                  title={
                    isLocked 
                      ? `Düşünen mod kilitli (Saat ${getLocalFormattedTime(thinkingCooldownUntil)}'a kadar limitiniz dolmuştur - ${formatRemainingTime(remainingMs)} kaldı)`
                      : model.description
                  }
                >
                  {isThinking ? (
                    ...
                    <div className="relative flex items-center justify-center">
                      {isSelected && (
                        <span className="animate-ping absolute inline-flex h-4 w-4 rounded-full bg-white opacity-40" style={{ animationDuration: '1.8s' }}></span>
                      )}
                      <Zap className={cn("w-4 h-4 relative z-10", isSelected ? "text-white fill-white/20" : "")} />
                    </div>
                  )}
                  <span>{model.name}</span>
                  {isLocked && isThinking && (
                    <span className="text-[10px] font-mono text-amber-400/90 ml-0.5 bg-amber-500/10 px-1 py-0.5 rounded border border-amber-500/20">
                      {formatRemainingTime(remainingMs)}
                    </span>
                  )}
                  {isSelected && !isThinking && (
                    <span className="relative flex h-1.5 w-1.5 ml-0.5">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-white opacity-80" style={{ animationDuration: '1.2s' }}></span>
                      <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-white shadow-[0_0_5px_#ffffff]"></span>
                    </span>
                  )}
                </button>
              );
